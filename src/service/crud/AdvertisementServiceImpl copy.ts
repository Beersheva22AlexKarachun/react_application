...
export default class AdvertisementServiceImpl {
  ...
  getAllAdvertisements(params: Parameter): Observable<Advertisement[]> {
    let intervalId: any;
    if (!this.observable) {
      this.observable = new Observable<Advertisement[]>(subscriber => {
        const getAllUrl = this.url + "/get_all" + getParams(params);
        this.cache.reset();
        this.sibscriberNext(getAllUrl, subscriber);
        intervalId = setInterval(() => this.sibscriberNext(getAllUrl, subscriber), POLLER_INTERVAL);
...
function getParams(params: Parameter): string {
  console.log(params)
  return Object.entries(params).reduce((res, entry) => {
